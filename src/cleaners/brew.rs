...
impl Cleaner for BrewCleaner {
    ...
    fn analyze(&self) -> Result<AnalysisResult> {
        ...
        if count > 0 {
            result.items.push(CleanItem {
                ...
                size_bytes: 0,
                removable: false,
                kind: CleanKind::Maintenance,
                risk: RiskLevel::Low,
                reason: "Informational only; use the update command to upgrade packages.".into(),
            });
        }
        Ok(result)
    }
    ...
    fn clean(&self, result: &AnalysisResult, dry_run: bool, yes: bool) -> Result<()> {
        ...
        if !yes && !ui::confirm("Run brew cleanup and autoremove?", false)? {
            return Ok(());
        }
